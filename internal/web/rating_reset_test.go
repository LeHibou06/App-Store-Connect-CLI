...
func TestRatingResetRequestRejectsMissingRelationship(t *testing.T) {
	for _, body := range []string{`{}`, `null`, `{"errors": []}`} {
		t.Run(body, func(t *testing.T) {
			...
			defer server.Close()
			_, err := testWebClient(server).GetAppStoreVersionRatingResetRequest(context.Background(), "version-123")
			if err == nil || !strings.Contains(err.Error(), "data member missing") {
				t.Fatalf("error = %v, want missing relationship error instead of an unscheduled reset", err)
			}
		})
	}
}
