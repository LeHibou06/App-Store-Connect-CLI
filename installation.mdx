...
<Steps>
  ...
  <Step title="Verify checksums">
    Validates the downloaded binary against the release SHA-256 checksums. The script aborts if verification cannot run (missing `checksums.txt`, missing `shasum`/`sha256sum`, or an unlisted asset); set `ASC_INSTALL_INSECURE=1` to explicitly skip verification in emergencies. A checksum mismatch always aborts.
  </Step>
...
<Note>
  ...
  reviewed 25-skill manifest in `internal/cli/install/install.go`.
</Note>
