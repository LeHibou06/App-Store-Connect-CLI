...
* `--allow-missing` - Return successful metadata with `available: false` instead of creating a file when no report exists for the requested date. Without this flag, the API error remains unchanged.
