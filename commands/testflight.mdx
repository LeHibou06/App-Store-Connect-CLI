...
The collection surface also supports exact ID and public-link filters:
...
Use `--include` with `--fields`, `--app-fields`, `--build-fields`,
...
Use an App Store Connect build ID to
...
The `--invite-type`, `--sort`, and `--include` flags are supported. `--sort`
