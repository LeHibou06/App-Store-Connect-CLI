...
Validate metadata files for errors. Validation is offline by default:
...
* `--dir` - Metadata root directory (required)
* `--check-urls` - Fetch support and privacy policy URLs to detect failed responses, redirects to another host, and site-root destinations
* `--subscription-app` - Enable subscription-specific Terms of Use / EULA link checks
* `--output` - Output format: `json`, `table`, `markdown`
* `--pretty` - Pretty-print JSON output

`--check-urls` is an explicit network opt-in. It checks each unique URL once
with bounded concurrency and timeouts, follows redirects, and refuses private,
loopback, link-local, and reserved network targets. URL findings are warnings:
they appear in the normal validation result but keep exit status 0 when the
metadata has no errors.
