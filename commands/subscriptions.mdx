...
Manage group display names through a subscription group version. Create or
resolve a group version and use its version ID; the product-scoped v1
`asc subscriptions groups localizations ...` commands were removed in 5.0.0.
...
There is no one-command sync for group localizations. Reconcile metadata with
the group version-localization `list`, `create`, `update`, and `delete`
commands.
...
<Note>
  ...
  ID is the only localization workflow for subscription groups. The
  group-scoped v1 `asc subscriptions groups localizations ...` commands were
  removed in 5.0.0 and now fail as unknown commands.
</Note>
...
product-scoped v1 `asc subscriptions localizations ...` and
`asc subscriptions images ...` command trees were removed in 5.0.0; use the
version-scoped commands below.
...
asc subscriptions pricing prices list --subscription-id "SUB_ID" --price-point-id "PRICE_POINT_ID"
asc subscriptions pricing prices list --subscription-id "SUB_ID" \
...
* `--territory` - Filter by a single territory
* `--price-point-id` - Filter by subscription price point IDs (comma-separated)
* `--fields` - Sparse fields for returned subscription prices
* `--territory-fields` - Sparse fields for included territories (`currency`)
* `--price-point-fields` - Sparse fields for included subscription price points
* `--include` - Include `territory` and/or `subscriptionPricePoint`
* `--limit` - Results per page (1-200)
* `--paginate` - Fetch all pages

The `--resolved` form keeps its fixed effective-price projection and accepts
only its existing `--plan-type` and `--territory` filters. It cannot be
combined with `--price-point-id`, sparse-field flags, or `--include`.
...
`--confirm` is required unless `--dry-run` is set. When both are set, `--dry-run` wins and nothing is created.
...
This is the public API-key command and the default for scripts and CI. It
prints Apple's unmodified JSON:API collection, so public responses can include
`proceeds`, `proceedsYear2`, relationship links, included territory currency,
and pagination metadata. It does not emit the web command's computed
`status`/`available` fields or its normalized `missingTerritories` result.

For Apple's web-session `/iris` MONTHLY matrix, especially when an equalization
failure needs the normalized missing-territory diagnostic, use
`asc web subscriptions pricing adjusted-equalizations view` instead. That
command requires a web session and does not require
`--upfront-price-point-id`; its output shape and 409 handling are intentionally
different.
...
<Note>
  ...
  `asc subscriptions pricing plan-availability` or
...
`subscriptionPlanAvailabilities` is the App Store Connect API 4.4 replacement
for the deprecated Subscription availability resource. Each subscription has one
plan availability per billing plan: `UPFRONT` and `MONTHLY`.

```bash  theme={null}
asc subscriptions pricing plan-availability show \
  --subscription-id "SUB_ID"

asc subscriptions pricing plan-availability set \
  --subscription-id "SUB_ID" \
  --plan-type UPFRONT \
...
`show` prints `GET /v1/subscriptions/{id}/planAvailabilities` with
`include=availableTerritories`. Apple caps the included linkages at 50 per plan
availability and reports the real count in the response paging metadata. When
that cap truncates the included list, `show` warns on stderr; do not pass the
truncated include to `set --territories`. `set` reads the complete relationship
before replacing it.

`set` is a desired-state write. The complete `--territories` list replaces the
plan's territories: territories missing from the list are removed and new ones
are added. Before writing, the CLI reads the plan's current territories through
`GET /v1/subscriptionPlanAvailabilities/{id}/relationships/availableTerritories`
and reports the diff; when the plan already matches the request it makes no
write and reports `changed: false`. After writing it re-reads the territories and
the plan availability resource and fails if they do not match the request.

**Flags:**

* `--plan-type` - Required: `UPFRONT` or `MONTHLY`
* `--territories` - Required: the complete desired territory list
* `--available-in-new-territories` - `UPFRONT` plans only
* `--confirm` - Required, because the write can remove territories

Emptying availability is rejected; removing a subscription from sale runs
through `asc web subscriptions availability remove-from-sale`. USA and Singapore
are removed from `MONTHLY` requests with a warning because Apple excludes those
storefronts.
...
query state. Use the version-scoped commands below for localization and image
writes.
...
version. Create or resolve a subscription version, then use its version ID; the
product-scoped `asc subscriptions localizations ...` commands were removed in
5.0.0.
...
There is no one-command sync for subscription localizations. Reconcile metadata
with the version-localization `list`, `create`, `update`, and `delete`
commands.

## Version-Scoped Promotional Images

Upload promotional images for subscription versions. Create or resolve a
subscription version and use the version image resource; the product-scoped
`asc subscriptions images ...` commands were removed in 5.0.0.
...
Use `--all-territories` to create the offer in every available territory.
`--territory` accepts a single territory only; `--territory ALL` is rejected as
an invalid territory, and combining `--territory` with `--all-territories` is
invalid.
...
# Import introductory offers from CSV (--confirm is required unless --dry-run is set)
