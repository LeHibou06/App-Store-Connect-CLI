...
* `duplicate` - Duplicate a workflow under a new name
...
* `--id` - Source workflow ID to copy (required)
* `--name` - Name for the new workflow (required)
* `--description` - Description for the new workflow (default: copied from the source workflow)
* `--enabled` - Enable the new workflow immediately; the copy is created disabled by default
* `--output` - Output format: `json`, `table`, `markdown`
* `--pretty` - Pretty-print JSON output

`duplicate` reads the source workflow with
`GET /v1/ciWorkflows/{id}?include=product,repository,xcodeVersion,macOsVersion` and
creates the copy with `POST /v1/ciWorkflows` in the same product, repository, Xcode
version, and macOS version. The copy is created disabled and unlocked so it can be
reviewed and edited before it starts building. TestFlight post-actions and workflow
environment variables are not part of the public workflow schema, so they are not
copied; configure those with `asc web xcode-cloud workflows edit` and
...
List and manage Xcode Cloud products:

```bash  theme={null}
asc xcode-cloud products
asc xcode-cloud products list --app "APP_ID"
asc xcode-cloud products list --product-type APP --include app --app-fields name,bundleId
...
* `--app` - App Store Connect app ID, bundle ID, or exact app name (or `ASC_APP_ID`)
* `--product-type` - Filter by `APP` or `FRAMEWORK`
* `--fields` - Sparse fields for CI products
* `--app-fields` - Sparse fields for included apps; requires `--include app`
* `--bundle-id-fields` - Sparse fields for included bundle IDs; requires `--include bundleId`
* `--scm-repository-fields` - Sparse fields for included SCM repositories; requires `--include primaryRepositories`
* `--include` - Include `app`, `bundleId`, or `primaryRepositories`
* `--primary-repositories-limit` - Maximum included primary repositories (1-50); requires `--include primaryRepositories`
* `--limit` - Maximum results per page (1-200)
* `--next` - Fetch next page using a links.next URL
* `--paginate` - Automatically fetch all pages
* `--output` - Output format: `json`, `table`, `markdown`
...
Query-shaping flags cannot be combined with `--next`; Apple's continuation URL
already contains the complete request query.
