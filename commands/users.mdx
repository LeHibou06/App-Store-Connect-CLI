...
* `--visible-app` - Filter by visible app ID(s), comma-separated
* `--sort` - Sort by one or more comma-separated expressions: `username`, `-username`, `lastName`, or `-lastName`
* `--fields` - User fields to include: `username`, `firstName`, `lastName`, `roles`, `allAppsVisible`, `provisioningAllowed`, `visibleApps`
* `--app-fields` - Fields to include for related apps (requires `--include visibleApps`)
* `--include` - Include related resources: `visibleApps`
* `--visible-apps-limit` - Maximum included visible apps (1-50; requires `--include visibleApps`)
