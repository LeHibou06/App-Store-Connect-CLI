...
the app is a personal service—are not checked because doctor uses an API key.
Doctor output reports this boundary under `coverageWarnings` in JSON and
under **Coverage Warnings** in table and Markdown output. Coverage warnings do
not change blocker counts or exit behavior. When no public-API blockers are
found, the next action still directs you to inspect these declarations with
`asc web apps declarations list --app APP_ID` before submission.

***

### `asc review submissions list`

This nested command path lists review submissions for an
app or globally.
